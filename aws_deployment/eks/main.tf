...
resource "aws_eks_node_group" "main" {
  ...
  tags = {
    ...
    "k8s.io/cluster-autoscaler.kubernetes.io/safe-to-evict" = false
  }
...
data "template_file" "kubeconfig" {
  ...
  vars = {
    ...
    cluster_arn               = "${data.aws_eks_cluster.cluster.arn}/${aws_eks_cluster.main.name}"
    region                    = var.region
    profile                   = var.profile
  }
}
...
resource "aws_iam_role_policy_attachment" "AmazonASGAutoscallingAllow" {
...
resource "aws_iam_policy" "cluster_autoscaler" {
  ...
  description = "EKS cluster-autoscaler policy for cluster ${aws_eks_cluster.main.id}"
...
data "aws_iam_policy_document" "cluster_autoscaler" {
  ...
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "autoscaling:ResourceTag/k8s.io/cluster-autoscaler/${aws_eks_cluster.main.id}"
      values   = ["owned"]
    }
